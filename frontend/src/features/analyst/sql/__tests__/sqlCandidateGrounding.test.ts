...
import {
  EMPTY_GROUNDED_RECOMMENDATION_COPY,
  createAppliedScopeFingerprint,
  getGroundedRecommendationEmptyState,
  getNeedsReviewGroundingCopy,
  groundCandidate,
  normalizeCandidates,
  summarizeGroundingSupport,
...
export function runSqlCandidateGroundingFixtures(): CandidateGroundingFixtureReport {
  ...
  const helperSummary = summarizeGroundingSupport([
    acceptedContractCandidate,
    recipeVerifiedCandidate,
    intentMismatchCandidate,
  ]);
  const needsReviewCopy = getNeedsReviewGroundingCopy(recipeVerifiedCandidate);
  const emptyStateNoScope = getGroundedRecommendationEmptyState({
    taskPrompt: "show leases expiring soon",
    appliedScopeTables: [],
    candidates: [],
  });
  const emptyStateNoPrompt = getGroundedRecommendationEmptyState({
    taskPrompt: "  ",
    appliedScopeTables: ["leases"],
    candidates: [],
  });
  const emptyStateNoSupportedMatches = getGroundedRecommendationEmptyState({
    taskPrompt: "show leases expiring soon",
    appliedScopeTables: ["leases"],
    candidates: [intentMismatchCandidate],
  });
  const emptyStateNeedsReviewOnly = getGroundedRecommendationEmptyState({
    taskPrompt: "show leases expiring soon",
    appliedScopeTables: ["leases", "tenants"],
    candidates: [recipeVerifiedCandidate],
  });
  const scopeFingerprint = createAppliedScopeFingerprint([
    "Tenants",
    "leases",
    "leases",
  ]);
  ...
  const results: CandidateGroundingFixtureResult[] = [
    ...
    expect(
      "empty-state copy exports all required strings",
      EMPTY_GROUNDED_RECOMMENDATION_COPY.noScope ===
        "Apply a worksheet scope to this tab before choosing templates." &&
        EMPTY_GROUNDED_RECOMMENDATION_COPY.noPrompt ===
          "Describe your task to see matching templates." &&
        EMPTY_GROUNDED_RECOMMENDATION_COPY.noSupportedMatches ===
          "No template matches your task in this scope yet. Try refining your prompt or applying additional worksheets." &&
        EMPTY_GROUNDED_RECOMMENDATION_COPY.needsReviewOnly ===
          "Possible matches found, but join keys are not yet verified. Review before running.",
      "Expected all four empty-state copy strings to be exported exactly.",
    ),
    expect(
      "needs-review copy helper returns warning-friendly output",
      needsReviewCopy.length === 1 && needsReviewCopy[0] === recipeJoinWarning,
      "Expected needs_review copy helper to surface join warning copy.",
    ),
    expect(
      "grounding support summary counts support states",
      helperSummary.supported === 1 &&
        helperSummary.needsReview === 1 &&
        helperSummary.unsupported === 1 &&
        helperSummary.bestSupport === "supported",
      "Expected support summary to count supported, needs_review, and unsupported candidates.",
    ),
    expect(
      "grounded recommendation empty-state helper chooses correct copy",
      emptyStateNoScope === EMPTY_GROUNDED_RECOMMENDATION_COPY.noScope &&
        emptyStateNoPrompt === EMPTY_GROUNDED_RECOMMENDATION_COPY.noPrompt &&
        emptyStateNoSupportedMatches ===
          EMPTY_GROUNDED_RECOMMENDATION_COPY.noSupportedMatches &&
        emptyStateNeedsReviewOnly ===
          EMPTY_GROUNDED_RECOMMENDATION_COPY.needsReviewOnly,
      "Expected empty-state helper to select noScope, noPrompt, noSupportedMatches, and needsReviewOnly copy.",
    ),
    expect(
      "applied scope fingerprint is stable and canonical",
      scopeFingerprint === "leases|tenants",
      "Expected applied scope fingerprint to lowercase, deduplicate, sort, and join scope tables.",
    ),
  ];
